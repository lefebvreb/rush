...
/// The zobrist keys used by polyglot. They are used in a bit of a strange way,
/// and are thus kept separate of the standard zobrist hashing of the main Rush
/// engine. Plus I really didn't want to change the way hashing was done.
const POLYGLOT_HASHING: [u64; 781] = [
...
fn polyglot_hash(board: &Board) -> u64 {
    ...
    for sq in board.get_occupancy().all().iter_squares() {
        ...
        hash ^= POLYGLOT_HASHING[64 * (usize::from(piece) * 2 + 1 - usize::from(color)) + usize::from(sq)];
    }

    // Side.
    if board.get_side_to_move() == Color::White {
        hash ^= POLYGLOT_HASHING[780];
    }

    // Castle rights.
    let rights = board.get_castle_rights();

    if rights.has(CastleMask::WhiteOO) {
        hash ^= POLYGLOT_HASHING[768];
    }
    if rights.has(CastleMask::WhiteOOO) {
        hash ^= POLYGLOT_HASHING[769];
    }
    if rights.has(CastleMask::BlackOO) {
        hash ^= POLYGLOT_HASHING[770];
    }
    if rights.has(CastleMask::BlackOOO) {
        hash ^= POLYGLOT_HASHING[771];
    }
...
impl BookEntry {
    ...
    pub fn mv(&self, board: &Board) -> Result<Move> {
        ...
        if matches!(self.from, Square::E1 | Square::E8) {
            let (_, piece) = board.get_piece(self.from).ok_or(Error::msg("Empty from square"))?;
            if piece == Piece::King {
                match to {
                    Square::A1 => to = Square::C1,
                    Square::H1 => to = Square::G1,
                    Square::A8 => to = Square::C8, 
                    Square::H8 => to = Square::G8,
                    _ => (),
                }
            }
        }
...
/// A struct that represents a move book, such as an opening book.
/// Books should be encoded in polyglot's format (commonly .bin).
/// See http://hgm.nubati.net/book_format.html for reference.
#[derive(Debug)]
pub struct Book {
    entries: Box<[BookEntry]>,
}
...
impl Book {
    ...
    pub fn open(path: &Path) -> Result<Book> {
        ...
        if bytes.len() % 16 != 0 {
            return Err(Error::msg("File has invalid size."))
        }

        let entries = bytes.as_slice().chunks(16).map(|chunk| {
            ...
            BookEntry {
                ...
                from: Square::SQUARES[usize::from(mv.wrapping_shr(6) & 0x3f)],
                to: Square::SQUARES[usize::from(mv & 0x3f)],
                maybe_promote: match mv.wrapping_shr(12) {
                    0 => None,
                    1 => Some(Piece::Knight),
                    2 => Some(Piece::Bishop),
                    3 => Some(Piece::Rook),
                    4 => Some(Piece::Queen),
                    _ => panic!("Unexpected promotion code."),
                },
            }
        }).collect();
